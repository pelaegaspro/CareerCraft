...
List<Map<String, dynamic>> generateTeamsDeterministic(Map<String, dynamic> payload) {
  ...
  while (generatedTeams.length < request.requestedCount && attempts < maxAttempts) {
    ...
    final leaders = assignCaptainViceCaptain(
      ...
      random: random,
      captainLockIds: request.exposureConfig.captainLockIds,
      captainAvoidIds: request.exposureConfig.captainAvoidIds,
    );
...
LeaderPair assignCaptainViceCaptain({
  ...
  // STEP 1 — Remove avoided players from captain candidates
  final candidates = team
      .where((p) => !captainAvoidIds.contains(p.id))
      .toList();

  if (candidates.length < 2) {
    final c = team.first;
    final vc = team.length > 1 ? team[1] : team.first;
    return LeaderPair(
      captainId: c.id,
      viceCaptainId: vc.id,
    );
  ...
  // STEP 2 — Score function (CORE LOGIC)
  double score(FantasyPlayer p) {
    double s = p.projectedScore;

    // Role boost
    if (p.role == 'AR') s *= 1.35;
    if (p.role == 'BAT') s *= 1.15;
    if (p.role == 'BOWL') s *= 1.10;

    // Differential boost
    if (p.ownership < 0.3) s *= 1.20;
    if (p.ownership > 0.7) s *= 0.85;
  ...
  // STEP 3 — Captain LOCK override
  for (final p in candidates) {
    if (captainLockIds.contains(p.id)) {
      final vc = candidates.firstWhere((x) => x.id != p.id);
      return LeaderPair(
        captainId: p.id,
        viceCaptainId: vc.id,
      );
  ...
  // STEP 4 — Pick top players
  FantasyPlayer captain = candidates[0];
  FantasyPlayer viceCaptain = candidates[1];

  // STEP 5 — Anti-duplicate variation
  if (candidates.length >= 3 && random.nextDouble() > 0.7) {
    captain = candidates[random.nextInt(3)];
    viceCaptain = candidates.firstWhere((p) => p.id != captain.id);
  }

  // STEP 6 — Avoid repeating same captain pair
  final pairKey = '${captain.id}-${viceCaptain.id}';

  if (usedPairs.contains(pairKey) && candidates.length >= 3) {
    captain = candidates[1];
    viceCaptain = candidates[2];
  }

  usedPairs.add('${captain.id}-${viceCaptain.id}');
