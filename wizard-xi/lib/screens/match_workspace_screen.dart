...
class MatchWorkspaceScreen extends ConsumerWidget {
  const MatchWorkspaceScreen({super.key, required this.match});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final generationState = ref.watch(
      teamGenerationControllerProvider(match.id),
    );
    final controller = ref.read(
      teamGenerationControllerProvider(match.id).notifier,
    );
    final lineupReady = ref.watch(lineupAvailableProvider(match.id));

    // Auto refresh guard
    lineupReady.whenData((isReady) {
      if (isReady && playersAsync.value?.isNotEmpty == true) {
        final players = playersAsync.value!;
        WidgetsBinding.instance.addPostFrameCallback((_) {
          controller.handleLineupAutoGenerate(players);
          print('[AUTO] Lineup detected → regenerated teams');
        });
      }
    });
          ...
          child: playersAsync.when(
            data: (players) {
              final exposureConfig = ExposureConfig.fromPlayers(
                ...
                captainLockIds: generationState.captainLockIds,
                captainAvoidIds: generationState.captainAvoidIds,
              );
                  ...
                  if (generationState.generatedTeams.isNotEmpty) ...[
                    ...
                    SizedBox(
                      ...
                      child: FilledButton.icon(
                        onPressed: () {
                          // Guard: ensure teams exist
                          if (generationState.generatedTeams.isEmpty) {
                            return;
                          }

                          final text = GeneratedTeam.exportMultiple(
                            generationState.generatedTeams,
                          );
                          ...
                          if (context.mounted) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                content: Text(
                                  'All ${generationState.generatedTeams.length} teams copied (max 20)',
                                ),
                              ),
                            );
                          }
                        ...
                        label: const Text('Copy All Teams'),
                      ),
                    ),
                    const SizedBox(height: 16),
                    _BatchToolbar(
                      ...
                      onCopyBatch: () async {
                        ...
                        if (context.mounted) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text('Current batch copied'),
                            ),
                          );
                        }
                      },
          ...
          children: [
            ...
            Wrap(
              ...
              children: topPlayers.take(8).map((player) {
                ...
                return ActionChip(
                  label: Text('${player.name} ${effective.round()}%'),
        ...
        child: Column(
          ...
          children: [
            ...
            ...previewPlayers.map((player) {
              ...
              final maxAllowed =
                  ((requestedCount * exposureConfig.maxExposureFor(player)) /
                          100)
                      .floor();
...
class _MessageCard extends StatelessWidget {
  const _MessageCard({required this.color, required this.message});
