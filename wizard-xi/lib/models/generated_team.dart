...
class GeneratedTeam {
  ...
  String get captainName => playerById(captainId)?.name ?? 'Unknown Captain';
  String get viceCaptainName => playerById(viceCaptainId)?.name ?? 'Unknown VC';
  ...
  /// Export multiple teams as a formatted string
  /// Useful for bulk copying and sharing
  static String exportMultiple(List<GeneratedTeam> teams) {
    if (teams.isEmpty) {
      return 'No teams to export';
    }

    final max = teams.length > 20 ? 20 : teams.length;
    final export = teams
        .take(max)
        .map((team) {
          final buffer = StringBuffer('TEAM ${team.teamNumber}\n');
          for (final role in AppConstants.roleOrder) {
            final names = team
                .playersForRole(role)
                .map((p) => p.name)
          ...
          buffer.writeln('C: ${team.captainName}');
          buffer.writeln('VC: ${team.viceCaptainName}');
          return buffer.toString().trimRight();
        })
        .join('\n\n-------------------\n\n');
